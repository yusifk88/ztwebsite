
<template>
  <div class="bg-grey-lighten-5">

  <v-container>
<v-row class="mt-5">
  <v-col cols="12" sm="12">
    <h2 class="text-center text-h2 mt-5 font-weight-bold hero-text">Our Services</h2>
    <P class="text-center">
      At ZeroTrust we offer an extensive set of services that cover all parts of your business when it comes to IT which includes infrastructure, security, Web and app development, cloud migration and hybrid workflow systems and more.
    </P>
  </v-col>
</v-row>
  <services-compnent extended>

</services-compnent>
  </v-container>
    <div style="background-color: #04244C">
      <v-container>
        <v-row>
          <v-col cols="12">
            <h1 class="hero-text text-center text-h4 font-weight-light">We would love to hear fom you</h1>
            <p class="text-center text-white mt-5">Contact us or give us a call on +1003994849493</p>
          </v-col>
          <v-col cols="12" class="text-center">
           <v-btn size="x-large" rounded variant="flat">Contacts us now<v-icon>mdi-arrow-right</v-icon></v-btn>
          </v-col>

        </v-row>
      </v-container>
    </div>
  </div>

<testimonials-components></testimonials-components>
  <mid-call-to-action-component></mid-call-to-action-component>
</template>

<script lang="ts">
import {defineComponent} from 'vue'

export default defineComponent({
  name: "services"
})
</script>

<style scoped>

</style>